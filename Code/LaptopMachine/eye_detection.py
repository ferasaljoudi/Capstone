...
# Load the model
model_path = "eye_detection_model.h5"
...
while True:
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            if left_eye_crop.size > 0 and right_eye_crop.size > 0:
                ...
                # Predict both eyes images with the model
                left_eye_predict = np.argmax(model.predict(left_eye_processed))
                right_eye_predict = np.argmax(model.predict(right_eye_processed))

                # The status will be "Closed" if both eyes are closed
                if left_eye_predict == 0 and right_eye_predict == 0:
                    eye_status = "Closed"
    ...
    if sleepy_detected:
        ...
        # Generate a beep (frequency: 1000 Hz, duration: 600 ms)
